...
from .entropy_core_2 import (
    entangled_entropy_core_2,
    # DEFAULT_PROCESS_BACKEND as DEFAULT_PROCESS_BACKEND_2,
)
...
class RandomizedEntangledEntropyComplex(TypedDict):
    ...
    degree: Optional[Union[tuple[int, int], int, list[int]]]
...
def randomized_entangled_entropy_2(
    shots: int,
    counts: list[dict[str, int]],
    ...
    if isinstance(pbar, tqdm.tqdm):
        pbar.set_description_str(
            f"Calculate specific degree or selected qubits: {degree_or_selected}."
        )
    (
        purity_cell_dict,
        ...
        shots=shots,
        counts=counts,
        degree_or_selected=degree_or_selected,
        backend=backend,
    )
    purity_cell_list: list[Union[float, np.float64]] = list(purity_cell_dict.values())
    ...
    quantity: RandomizedEntangledEntropyComplex = {
        ...
        "puritySD": purity_sd,
        "entropySD": entropy_sd,
        "degree": degree_or_selected,
        "measureActually": (0, num_qubits),
        "bitStringRange": selected_qubits_sorted,
        "countsNum": len(counts),
        "takingTime": taken,
    }
...
def randomized_entangled_entropy_mitigated_2(
    shots: int,
    counts: list[dict[str, int]],
    degree_or_selected: Optional[Union[tuple[int, int], int, list[int]]],
    backend: PostProcessingBackendLabel = DEFAULT_PROCESS_BACKEND,
    ...
    if existed_all_system is None:
        ...
        source = "independent"
        bitstring_range_allsys = selected_qubits_sorted_allsys
        measure_range_allsys = (0, num_qubits)
    else:
        for k, msg in [
            ("purityCellsAllSys", "purityCellsAllSys is not in existed_all_system."),
            ("bitStringRange", "bitStringRange is not in existed_all_system."),
    ...
    quantity: RandomizedEntangledEntropyMitigatedComplex = {
        ...
        "degree": degree_or_selected,
        "num_qubits": num_qubits,
        "measure": ("This post-processing does not support measure range.", (0, num_qubits)),
        "measureActually": (0, num_qubits),
        "measureActuallyAllSys": measure_range_allsys,
        "countsNum": len(counts),
        "takingTime": taken,
