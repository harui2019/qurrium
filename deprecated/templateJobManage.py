...
class EntropyMeasure:
    ...
    def jobOnly(
        ...
        Returns:
            Union[dict, list[dict]]: Construnct the quantum computing jlb.
        """
        ...
        if type(backend) == IBMQBackend:
            jobIBMQ = IBMQJobManager().run(
                self.base[tgtExpId]['circuit'],
                backend,
                shots=shots,
                name=f'{quickStr(aNum, paramsOther)}'
            )
            self.base[tgtExpId]['jobIBMQ'] = jobIBMQ.job_set_id()
            ...
            self.base[tgtExpId]['jobAer'] = jobAer.job_id()
    ...
    def reset(
        ...
        if fuse:
            ...
            print(
                "Reset does not fulfill to prevent reset accidentally, " +
